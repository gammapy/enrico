...
class ModelTester(Loggin.Message):
    ...
    def __init__(self, config):
        ...
        self.FitRunner,self.Fit,self.ListOfAnalysisObjects = GenAnalysisObjects(config)
    ...
    def _setPowerLaw(self,comp,name,pars=None):
        ...
        SrcSpectrum.getParam('Prefactor').setFree(True)

        SrcSpectrum.getParam('Index').setBounds(-5,0)
        SrcSpectrum.getParam('Index').setValue(-2)
        SrcSpectrum.getParam('Index').setFree(True)
        ...
        if pars[1]!=None:
            ...
            SrcSpectrum.getParam('Index').setFree(False)
            par = comp.par_index(name, 'Prefactor')
            comp-freeze(par)
    ...
    def _setPLSuperExpCutoff(self,comp,name,pars=None):
        ...
        if pars[0]!=None:
            print("Fixing Prefactor")
            SrcSpectrum.getParam('Prefactor').setFree(False)
            SrcSpectrum.getParam('Prefactor').setValue(pars[0]/1e-11)
            par = comp.par_index(name, 'Prefactor')
            comp.freeze(par)
        if pars[1]!=None:
            ...
            SrcSpectrum.getParam('Index1').setFree(False)
        ...
        if pars[3]!=None:
            ...
            SrcSpectrum.getParam('Cutoff').setFree(False)
